...
function medform_main()
{
    require_once 'sqlstuff.php';
    require_once 'sqlsts.php';
    require_once 'medfunctions.php';

    $fn = "ERROR";
    $ln = "NO_CADET_SELECTED";
    $conn=connectDB();
  ...
  if(!$result1){
    ...
    $read3="SELECT tblMeds.MedID, tblMeds.MedName, tblMeds.Dose, tblMeds.fkMedType, tblMeds.Frequency, tblMeds.TakenWhen, tblMeds.MedStartDate, tblMeds.MedEndDate, tblMeds.MedNote
    FROM tblMeds
    JOIN tblClassDetails ON tblMeds.fkClassDetailID=tblClassDetails.ClassDetailID
    WHERE ClassDetailID=\"$classdetailid\"";
  ...
  if(!$result3){
    ...
    if($rows == 0)
    {
      echo "Cadet has no medicine on file. Click below to create entry.";
    }
    for($i=0;$i<$rows;$i++)
    {
      $row=$result3->fetch_array(MYSQLI_ASSOC);

      show($row,$i,$classdetailid, $rows);
    }
    ...
    echo <<<_END
      <form>
        <div class="form-group row">
          <div class="col-1">
          <button type="button" id="medinsert" class="btn btn-primary" onclick="insertmed($classdetailid, $rows)">INSERT</button>
          </div>
        </div>
      </form>
_END;
