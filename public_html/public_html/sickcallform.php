...
function sickcallform_main()
{
    require_once 'sqlstuff.php';
    require_once 'sickfunctions.php';
  ...
  if(!$result1){
    ...
    $read3="SELECT tblMedSickCalls.* 
      FROM tblMedSickCalls 
      JOIN tblClassDetails ON tblMedSickCalls.fkClassDetailID=tblClassDetails.ClassDetailID
  ...
  if(!$result3){
    ...
    if($rows == 0)
    {
      echo "Cadet has no sick calls on file. Click below to create entry.";
    }
    for($i=0;$i<$rows;$i++)
    {
      $row=$result3->fetch_array(MYSQLI_ASSOC);

      showSickCall($row,$i,$classdetailid, $rows);
    }
    ...
    echo <<<_END
      <form>
        <div class="form-group row">
          <div class="col-1">
          <button type="button" id="sickinsert" class="btn btn-primary" onclick="insertsick($classdetailid, $rows)">INSERT</button>
          </div>
        </div>
      </form>
_END;
